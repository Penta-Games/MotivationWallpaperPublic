...
import com.app.materialwallpaper.utilities.GDPR;
import com.app.materialwallpaper.utilities.ItemOffsetDecoration;
import com.app.materialwallpaper.utilities.Tools;
...
public class FragmentFeatured extends Fragment {
    ...
    private void firstLoadData() {
        ...
        if (tools.isNetworkAvailable()) {
            ...
            dbHelper.deleteData(Constant.TABLE_FEATURED);

            JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(Request.Method.GET, Constant.URL_FEATURED_WALLPAPER + 0, null, new Response.Listener<JSONArray>() {
                @Override
                public void onResponse(JSONArray response) {
                    ...
                    for (int i = 0; i < response.length(); i++) {
                        try {
                            ...
                            dbHelper.saveWallpaper(arrayList.get(i), Constant.TABLE_FEATURED);
            ...
            showRefresh(false);
            arrayList = dbHelper.getFeatured(Constant.TABLE_FEATURED);
            mAdapter = new AdapterWallpaper(getActivity(), arrayList);
            recyclerView.setAdapter(mAdapter);
        }
    ...
    private void loadMore() {
        ...
        JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(Request.Method.GET, Constant.URL_FEATURED_WALLPAPER + lastId, null, new Response.Listener<JSONArray>() {
            @Override
